...
import { Loader2, User, Lock, ArrowLeft, Mail } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await api.post<any>("/auth/login", { email, password, rememberMe });
      setUser(user);
    } catch (err: any) {
        ...
        <div className="flex flex-col justify-center flex-1 px-8 py-10 md:px-12">
            ...
            <>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <label className="flex items-center gap-2.5 cursor-pointer select-none pl-1">
                  <div className="relative">
                    ...
                    <div
                      ...
                      className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${rememberMe ? "bg-[#1A6DB5] border-[#1A6DB5]" : "bg-white border-gray-300"}`}
                    >
                      {rememberMe && (
                ...
                <button
                  ...
                  className="w-full h-11 rounded-full bg-[#1A6DB5] hover:bg-[#155ea0] disabled:opacity-50 text-white font-bold text-sm tracking-widest uppercase transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2"
                >
              ...
              <div className="mt-5 text-center">
                ...
                <button
                  ...
                  className="text-xs text-[#1A6DB5] font-semibold mt-0.5 hover:underline"
                >
                  Get help signing in.
                </button>
              </div>
          ...
          {view === "forgot" && (
            <>
              {!forgotSent ? (
                <>
                  <div className="w-8 h-0.5 bg-[#F5A800] mb-4" />
                  <h2 className="text-xl font-bold text-gray-800 mb-2 font-sora">Forgot your password?</h2>
                  <p className="text-xs text-gray-500 mb-6 leading-relaxed">
                    Enter your account email address below. If it matches an active user, we will send a password reset link to that email.
                  </p>
                  ...
                  <form onSubmit={handleForgot} className="space-y-4">
                    <div className="relative">
                      <input
                        ...
                        autoFocus
                        autoComplete="username"
                        className="w-full h-11 pl-4 pr-11 rounded-full border border-gray-200 bg-gray-50 text-sm text-gray-700 placeholder-gray-400 outline-none focus:border-[#F5A800] focus:ring-2 focus:ring-[#F5A800]/20 transition"
                      />
                    ...
                    <button
                      ...
                      className="w-full h-11 rounded-full bg-[#F5A800] hover:bg-[#d99300] disabled:opacity-50 text-white font-bold text-sm tracking-widest uppercase transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2"
                    >
                ...
                <>
                  ...
                  <div className="flex flex-col items-center text-center py-4">
                    ...
                    <h3 className="text-base font-bold text-gray-800 mb-2">Check your inbox</h3>
                    <p className="text-xs text-gray-500 leading-relaxed max-w-xs">
                      If <span className="font-medium text-gray-700">{forgotEmail}</span> matches an active account, a password reset link has been sent.
                    </p>
                  </div>
                </>
              )}
              ...
              <button
                ...
                className="mt-6 flex items-center gap-1.5 text-xs text-gray-400 hover:text-[#1A6DB5] transition mx-auto"
              >
